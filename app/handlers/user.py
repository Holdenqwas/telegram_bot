from telebot.async_telebot import AsyncTeleBot
from telebot.types import Message

from app.keyboards import menu
from app.states.menu import UserStateMenu
from app.services import user

state = UserStateMenu()


async def start_menu(message: Message, bot: AsyncTeleBot):
    await user.create_user(message.from_user.id, message.from_user.username)
    await bot.send_message(
        message.chat.id,
        "Вас приветствует личный бот помощник! Чтобы получить помощь \
воспользуйтейсь командой /help",
        reply_markup=menu.main_menu.markup,
    )


async def help_menu(message: Message, bot: AsyncTeleBot):
    await bot.send_message(
        message.chat.id,
        """Бот помощник в Telegram: Инструкция

Здесь ты найдешь все, что необходимо мне для комфортной жизни и многое другое.

 - Основные функции:
Тренировки

1. Хранилище тренировок:

До 5 различных видов тренировок: создай свои собственные планы тренировок.
До 10 упражнений на каждую тренировку: заполни названия упражнений, которые \
ты будешь выполнять.
Статистика по весу: бот запоминает твой вес
Отслеживание прогресса: бот напомнит тебе вес, с которым ты работал на прошлой \
тренировке.


2. Создание тренировки:

Нажми кнопку "Начать тренировку".
Введи свой текущий вес.
Выбери тренировку, которую ты хочешь провести.
Для каждого упражнения введи текущий вес, с которым ты будешь работать.

 - Как пользоваться:

1. Заполнение хранилища тренировок:

Создай новую тренировку:
Нажми кнопку "Настройка тренировок".
Здесь сперва создай тренировки.
Добавь упражнения, нажав кнопку "Названия упражнений" и введя название.
 Важно! "Удалить тренировки" Удалит полностью все записи о тренировках.

2. Проведение тренировки:

Нажми кнопку "Начать тренировку".
Введи свой текущий вес.
Выбери тренировку.
Для каждого упражнения:
Бот напомнит тебе вес, с которым ты работал на прошлой тренировке.
Введи текущий вес, с которым ты будешь работать.
После завершения тренировки бот сохранит твои результаты и обновит статистику.

Удачи в тренировках!


Покупки

1. Создавай свои списки или подключайся к спискам других пользователей

Для создания своего списка нажми на кнопку Добавить новый список и введи \
название списка.
Он станет доступен в меню Открыть список

Чтобы поделиться списком или просоединиться к чужому списку нужно,
нажать на пункт меню поделиться списком, переслать полученное сообщение с \
инсnрукцией и ID дургому пользователю.
В меню Присоединиться к списку другого пользователя вставить ID из сообщения.

2. Быстро добавляй предметы в списки и удаляй их.

Чтобы добавить предметы в список нужно нажать на Открыть список, выбрать \
нужный списко,
а дальше просто отправить текстом список продуктов, где каждый предмент на \
новой строке, например:

пиво 2 банки
чипсы с солью
попкорн
кола

Чтобы удалить предмет из списка, нужно нажать на его название в клавиатуре

3. Можно привязать к Алисе от Яндекса

Сказать нужно:
 - в боте в нажать Покупки -> Связать список с Яндекс станцией
 - сказать Ализа запусти навык Совместный список покупок, нажать Авторизаться
 - ввести два числа в форму и можно пользоваться

Предметы перечислять словом запятая, например "пиво 2 банки запятая чипсы"

Об авторе: /author""",
    )


async def author_menu(message: Message, bot: AsyncTeleBot):
    await bot.send_message(
        message.chat.id,
        """Обо мне

Я - Антон, создатель этого бота для разного (v1.1.0).

Обратная связь

Если у вас есть пожелания или предложения по доработке моего бота, пожалуйста, \
пишите мне в личку: @akhorn

В ближайших планах по развитию бота:

 - Статистика по тренировка с графиками
 - Черный список для продуктов: планируется реализовать функцию черного списка \
для продуктов, которая позволит блокировать или ограничивать доступ к \
определенным товарам.

Следите за обновлениями!

Changelog:

09.10.2024 v1.0.0
 - Добавлен раздел тренировок, добавление до 5 тренировок, до 10 упражнений, \
сохранение веса.

13.10.2024 v1.1.0
 - Добавлен раздел покупок, бесконечные списки покупок, которые можно вести \
совместно с другими.

05.12.2024 v1.2.0
 - Добавлен привязку к Яндекс станции, навык назвается Совметсный список \
покупок.
""",
)


async def get_main_menu(message: Message, bot: AsyncTeleBot):
    state.clear_mem(message.from_user.id)
    await bot.send_message(
        message.chat.id,
        "Главное меню",
        reply_markup=menu.main_menu.markup,
    )
    return


async def main_menu(message: Message, bot: AsyncTeleBot):
    if message.text == "Тренировка":
        state.push_menu(message.from_user.id, menu.train_menu.name)
        await bot.send_message(
            message.chat.id,
            menu.train_menu.title,
            reply_markup=menu.train_menu.markup,
        )
        return
    elif message.text == "Покупки":
        state.push_menu(message.from_user.id, menu.shop_menu.name)
        await bot.send_message(
            message.chat.id,
            menu.shop_menu.title,
            reply_markup=menu.shop_menu.markup,
        )
        return
    await bot.send_message(
        message.chat.id, "Главное меню", reply_markup=menu.main_menu.markup
    )
